...
#pragma ModuleName=GetBrickletRawData
...
static Function complains_invalid_range()
	...
	first = -1
	last  = -1
	MFR_GetBrickletRawData/R=(first,last)
	...
	variable index
	index = -1
	MFR_GetBrickletRawData/R=(index)
	CHECK_EQUAL_VAR(err.INVALID_RANGE,V_flag)
	CHECK_EMPTY_STR(S_waveNames)
	CHECK_EMPTY_FOLDER()
...
static Function complains_wavename_toolong()
	...
	MFR_GetBrickletRawData/N="abcdefghijklmnopqrstuvwxyz"
	CHECK_EQUAL_VAR(err.WRONG_PARAMETER,V_flag)
	CHECK_EMPTY_STR(S_waveNames)
	CHECK_EMPTY_FOLDER()
End
...
static Function complains_empty_df()
	...
	dfref df
	MFR_GetBrickletRawData/DEST=df
	CHECK_EQUAL_VAR(err.WRONG_PARAMETER,V_flag)
	CHECK_EMPTY_STR(S_waveNames)
	CHECK_EMPTY_FOLDER()
End
...
static Function creates_one_wave()
	...
	MFR_OpenResultFile/K folder + file
	CHECK_EQUAL_VAR(err.SUCCESS,V_flag)
	MFR_GetBrickletRawData/R=(1)
	CHECK_EQUAL_VAR(err.SUCCESS,V_flag)
	CHECK_WAVE($":X_00001:rawData_00001", NUMERIC_WAVE, minorType=INT32_WAVE)

	CHECK(ItemsInList(S_waveNames) == 1)
	CHECK_WAVE($StringFromList(0,S_waveNames), NUMERIC_WAVE, minorType=INT32_WAVE)
...
static Function creates_one_wave_df_name()
	...
	NewDataFolder abcd
	dfref df = abcd
	MFR_GetBrickletRawData/DEST=df/N="myWave"/R=(1)
	CHECK_EQUAL_VAR(err.SUCCESS,V_flag)
	CHECK_WAVE($":abcd:X_00001:myWave_00001", NUMERIC_WAVE, minorType = INT32_WAVE)
...
static Function creates_one_wave_free_df()
	...
	MFR_GetBrickletRawData/DEST=df/R=(1)
	CHECK_EQUAL_VAR(err.SUCCESS,V_flag)
	Wave/SDFR=df wv = $":X_00001:rawData_00001"
	CHECK_WAVE(wv, NUMERIC_WAVE, minorType = INT32_WAVE)
...
static Function creates_waves()
	...
	for(i = 1; i <= numBricklets; i+=1)
		string wvName
		sprintf wvName, "rawData_%05d", i
		CHECK_WAVE($wvName, NUMERIC_WAVE, minorType = INT32_WAVE)
	endfor

	CHECK(ItemsInList(S_waveNames) == numBricklets)

	for(i = 0; i < ItemsInList(S_waveNames); i+=1)
		CHECK_WAVE($StringFromList(i,S_waveNames), NUMERIC_WAVE, minorType = INT32_WAVE)
	endfor
End
...
static Function does_not_overwrite_by_default()
	...
	MFR_GetBrickletRawData/R=(1,2)
	CHECK_EQUAL_VAR(err.WAVE_EXIST,V_flag)
	CHECK_EMPTY_STR(S_waveNames)
	CHECK(!WaveExists($":X_00002:rawData_00002"))
End
...
static Function setting_datafolder()
	...
	MFR_GetBrickletRawData/R=(1)
	CHECK_EQUAL_VAR(err.SUCCESS,V_flag)
	CHECK_WAVE($"rawData_00001", NUMERIC_WAVE, minorType = INT32_WAVE)
...
static Function prefers_dfref_settings()
	...
	MFR_GetBrickletRawData/R=(1)/DEST=dfr
	CHECK_EQUAL_VAR(err.SUCCESS,V_flag)
	Wave/SDFR=dfr wv1 = $":X_00001:rawdata_00001"
	CHECK_WAVE(wv1, NUMERIC_WAVE, minorType = INT32_WAVE)
...
static Function liberal_wave_name_works()
	...
	MFR_GetBrickletRawData/N="a b"/R=1
	CHECK_EQUAL_VAR(err.SUCCESS, V_flag)
